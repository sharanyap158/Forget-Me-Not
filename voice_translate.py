...
with st.expander("🌍 Translate Last Voice Input"):
    translator = RealTimeTranslator()

    st.markdown("You can translate the last recorded voice input into another language and listen to it.")

    # Language selection
    target_lang = st.selectbox("Choose target language:", options=list(translator.languages.keys()), format_func=lambda x: translator.languages[x])

    if st.button("🌐 Translate Last Recording"):
        audio_file = "assistant_reply.mp3"

        if not os.path.exists(audio_file):
            st.warning("No recorded audio found. Please record first.")
        else:
            # Transcribe to English (assumed source)
            original_text = transcribe_audio(audio_file)
            st.markdown(f"**Original (English):** {original_text}")

            # Translate to selected language
            translated_text = translator.translate_text(original_text, "en", target_lang)
            st.markdown(f"**Translated ({translator.languages[target_lang]}):** {translated_text}")

            # Convert to speech and play
            tts_path = translator.text_to_speech(translated_text, target_lang)
            if tts_path:
                st.audio(tts_path)
            else:
